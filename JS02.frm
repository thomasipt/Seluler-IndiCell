...
Begin VB.Form JS02 
   ...
   Begin ACTIVESKINLibCtl.Skin Skin1 
      Left            =   9360
      OleObjectBlob   =   "JS02.frx":05C5
      Top             =   930
   End
   Begin ACTIVESKINLibCtl.SkinLabel SkinLabel1 
      Height          =   195
      Left            =   120
      OleObjectBlob   =   "JS02.frx":07F9
      TabIndex        =   14
      Top             =   450
      Width           =   1035
   End
...
If RKTG.RowCount <> 0 Then
   ...
   Loop
Else
    MsgBox "KODE SPAREPART MASIH KOSONG", vbCritical, "KONFIRMASI"
    Call Buka
...
'If Text11 = 0 Then
'    MsgBox "UANG MUKA MASIH KOS0NG", vbCritical, "WARNING"
'    Exit Sub
'End If

If Text10 = 0 Then
    MsgBox "TOTAL TRANSAKSI KOSONG", vbCritical, "WARNING"
    Exit Sub
End If
...
tanya = MsgBox("TRANSAKSI SELESAI", vbOKCancel, "KONFIRMASI")
    If tanya = vbOK Then
        Text12 = CCur(Text10) - CCur(Text11)
...
If RSave.RowCount <> 0 Then
    RSave.Edit
    RSave("MutasiD") = CCur(Text30) + CCur(Text11)
    RSave("Saldo") = CCur(Text32) + CCur(Text11)
    ...
    RSave2.AddNew
        ...
        RSave2("Keterangan") = "SERVICE NOTA " + Trim(Text1) + " / " + Trim(Text2)
        RSave2("NominalD") = CCur(Text11)
        RSave2("NominalC") = 0
        RSave2("Saldo") = CCur(Text32) + CCur(Text11)
        RSave2("Tanggal") = Date
        RSave2("Jam") = Time
...
Set RSave = RDCO.OpenResultset(SSave, rdOpenDynamic, rdConcurRowVer)
    ...
    SCari = "Select * From TOTALJS02 where NOTA= '" + Trim(Text1) + "'"
    ...
    If RCari.RowCount <> 0 Then
        ...
        RSave("TUntung") = CCur(RCari("SumOfLABA"))
        RSave.Update
...
If RKTG.RowCount <> 0 Then
   ...
   Loop
Else
    MsgBox "KODE SERVICE MASIH KOSONG", vbCritical, "KONFIRMASI"
    Call Buka
...
Else
    ...
    Set RSave = RDCO.OpenResultset(SSave, rdOpenDynamic, rdConcurRowVer)
        ...
        SCari = "Select * From B003 where KodeBR= '" + Trim(Text17) + "'"
        Set RCari = RDCO.OpenResultset(SCari, rdOpenDynamic, rdConcurRowVer)
        If RCari.RowCount <> 0 Then
                ...
                RSave("LABA") = CCur(RCari("HJual") - RCari("HBeli")) * CCur(Text15)
                RSave.Update
...
Else
    ...
    Set RSave = RDCO.OpenResultset(SSave, rdOpenDynamic, rdConcurRowVer)
        ...
        SCari = "Select * From JS01 where Kode= '" + Trim(Text24) + "'"
        Set RCari = RDCO.OpenResultset(SCari, rdOpenDynamic, rdConcurRowVer)
        If RCari.RowCount <> 0 Then
            ...
            RSave("LABA") = CCur(RCari("Biaya") - RCari("Komisi"))
            RSave.Update
...
If RCari.RowCount <> 0 Then
    Text30 = Format(RCari("MutasiD"), "##,###.00")
    Text31 = Format(RCari("MutasiC"), "##,###.00")
    Text32 = Format(RCari("Saldo"), "##,###.00")
End If
RCari.Close
Set RCari = Nothing
...
If Text11 = "" Then
    Text11 = "0.00"
    Text12 = Format(CCur(Text10), "##,###.00")
ElseIf CCur(Text10) < CCur(Text11) Then
