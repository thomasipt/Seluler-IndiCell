...
Else
    ...
    If tanya = vbOK Then
        TTL = Format(CCur(Text8) + (CCur(Text6) * CCur(Text7)), "##,###.00")
        If CCur(TTL) > CCur(Text4) Then
            MsgBox "SALDO KAS TIDAK CUKUP", vbSystemModal, "KONFIRMASI"
            Exit Sub
        Else
            Text8 = Format(CCur(TTL), "##,###.00")
            Call SimpanBL001
            Call SiapkanGrid
            Call IsiGrid
            Call Auto
...
If RSave.RowCount <> 0 Then
    CREDIT = RSave("MutasiC")
    SALDO = RSave("Saldo")
    RSave.Edit
    RSave("MutasiC") = CCur(CREDIT) + CCur(Text8)
    RSave("Saldo") = CCur(SALDO) - CCur(Text8)
    ...
    RSave2.AddNew
        ...
        RSave2("Keterangan") = "PEMBELIAN FAK." + Trim(Text1) + "  NO." + Trim(Text3)
        RSave2("NominalD") = 0
        RSave2("NominalC") = CCur(Text8)
        RSave2("Saldo") = CCur(SALDO) - CCur(Text8)
        RSave2("Tanggal") = Date
        RSave2("Jam") = Time
...
If RCari.RowCount <> 0 Then
    Text9 = Format(RCari("MutasiD"), "##,###.00")
    Text5 = Format(RCari("MutasiC"), "##,###.00")
    Text4 = Format(RCari("Saldo"), "##,###.00")
End If
RCari.Close
Set RCari = Nothing
